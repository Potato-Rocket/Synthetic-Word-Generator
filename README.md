...
A simple, command-line Python script which generates natural sounding nonsense words. Given sample text such as books, articles, etc., this script analyzes the letter frequencies in the words present in the text to inform word generation.
...
3. Builds a Markov chain model based on the list of unique words. Describes, for some *n* preceding letters (*n* being the "context length" of the model), what the probability of each possible subsequent letter is. The start and end of each word are demarcated by special characters.
...
- `-l` or `--context_length`: The context length of the model
...
- Saving and loading pre-calculated models may make the script more useful for general utility. For now, perhaps things can be stored in a data folder by default.
